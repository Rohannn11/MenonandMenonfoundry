...
def _init_capa_state():
    if "capa_register" not in st.session_state:
        st.session_state["capa_register"] = []
    if "capa_seq" not in st.session_state:
        st.session_state["capa_seq"] = 1
    if "capa_prev_source" not in st.session_state:
        st.session_state["capa_prev_source"] = None
    if "capa_prev_edit" not in st.session_state:
        st.session_state["capa_prev_edit"] = None

def _get_capa_snapshot():
    _init_capa_state()
    register = st.session_state["capa_register"]
    today = date.today()
        ...
        if current_state:
            ...
            with scada_tabs[2]:
                ...
                if selected_source != "Manual Entry":
                    ...
                    severity = parts[0] if len(parts) > 0 else "WARN"
                    title = parts[1] if len(parts) > 1 else selected_source
                    default_issue = title
                    linked_alert = title
                    default_priority = "HIGH" if severity == "CRITICAL" else "MEDIUM"

                # Preserve user inputs across auto-refresh/reruns using session_state keys.
                # Only apply alert-derived defaults when the selected source changes.
                if selected_source != st.session_state.get("capa_prev_source"):
                    st.session_state["capa_issue"] = default_issue
                    st.session_state["capa_priority"] = default_priority
                    st.session_state["capa_source"] = "ALERT" if selected_source != "Manual Entry" else "MANUAL"
                    st.session_state["capa_due_date"] = date.today() + (
                        timedelta(days=3) if default_priority == "HIGH" else timedelta(days=7)
                    )
                    st.session_state["capa_prev_source"] = selected_source
